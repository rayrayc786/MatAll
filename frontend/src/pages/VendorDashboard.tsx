...
const VendorDashboard: React.FC = () => {
  ...
  const fetchAvailableOrders = async () => {
    const token = localStorage.getItem('token');
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/vendor/orders/available`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAcceptOrder = async (orderId: string) => {
    const token = localStorage.getItem('token');
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/vendor/orders/${orderId}/accept`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleStatusUpdate = async (orderId: string, status: string) => {
    ...
    try {
      ...
      loadAll();
      if (status === 'vendor-confirmed') toast.success('Order confirmed!');
  ...
  return (
    <main className="content vendor-dashboard" style={{ maxWidth: '1600px', margin: '0 auto', padding: '2rem' }}>
      ...
      <div className="orders-tabs" style={{ display: 'flex', gap: '2rem', borderBottom: '2px solid #f1f5f9', marginBottom: '2.5rem' }}>
        ...
        >
          My Active Tasks ({orders.filter(o => !['delivered', 'rejected-by-vendor', 'cancelled'].includes(o.status)).length})
        </button>
        <button 
